...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 08:07:51 2024

@author: meganfairchild

I asked chat GPT to write this for me to look at optimized code
"""
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if i + 3 < rows and j + 3 < cols:
                diagonal_dr = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
                max_product = max(max_product, diagonal_dr)

            # Diagonal product (down-left)
            if i + 3 < rows and j - 3 >= 0:
                diagonal_dl = grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
                max_product = max(max_product, diagonal_dl)
...
# Find and print the largest product
print("The largest product is:", largest_grid_product(grid))

"""
Congratulations, the answer you gave to problem 11 is correct.

You are the 250064th person to have solved this problem.
